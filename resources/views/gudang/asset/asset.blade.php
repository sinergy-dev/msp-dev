    ...
    <div class="alert alert-success" id="alert">
      ...
      <div class="card mb-3">
        <div class="card-header">
           <i class="fa fa-table"></i>&nbsp<b>Asset Management</b>
        </div>
        <div class="card-body">
          <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            <li class="nav-item">
              <a class="nav-link" id="profile-tab" data-toggle="tab" href="#kembaliin" role="tab" aria-controls="kembaliin" aria-selected="false"> List Pinjaman</a>
            </li>
            @endif -->
          </ul>
          <div class="tab-content" id="myTabContent">
            <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
              <br>
              <!-- <button class="btn btn-primary-lead margin-bottom" id="" data-target="#add_asset" data-toggle="modal"><i class="fa fa-plus"> </i>&nbsp Asset</button> -->
                ...
                <table class="table table-bordered nowrap " id="data_Table" width="100%" cellspacing="0">
                  ...
                  <tbody id="products-list" name="products-list">
                    ...
                    <tr>
                      ...
                      <td>
                        @if($data->qty == 0)
                        <button class="btn btn-md disabled" style="background-color: grey">Peminjaman</button>
                        @else
                        <button class="btn btn-md btn-warning" onclick="pinjam('{{$data->id_barang}}','{{$data->nama}}','{{$data->qty}}')" data-target="#peminjaman" data-toggle="modal" >Peminjaman 
                        </button>
                        @endif
                      </td>
                      @if(Auth::User()->id_division == 'WAREHOUSE')
                      <td>
                      	<a href="{{url('/detail_asset', $data->id_barang) }}"><button class="btn btn-sm sho" style="width: 150px;">Detail Peminjaman</button></a>
                      </td>
                      @endif
                    </tr>
                    @endforeach
                ...
                <table class="table table-bordered nowrap DataTable" id="datatable" width="100%" cellspacing="0">
                  ...
                  <tbody id="products-list" name="products-list">
                    ...
                    <tr>
                      ...
                      <td>
                      	@if($data->status == 'PENDING')
                        <button class="btn btn-md btn-success" data-target="#accept_modal" data-toggle="modal" onclick="id_accept_update('{{$data->id_transaksi}}','{{$data->id_barang}}')">ACCEPT</button>
                        <button class="btn btn-md btn-danger" data-target="#reject_modal" data-toggle="modal" onclick="id_reject_update('{{$data->id_transaksi}}','{{$data->id_barang}}')">REJECT</button>
                        @elseif($data->status == 'ACCEPT')
                        <button class="btn btn-md btn-danger" data-target="#kembali_modal" data-toggle="modal" onclick="kembali('{{$data->id_barang}}','{{$data->id_transaksi}}')" style="text-align: center;width: 125px">KEMBALI</button>
                        <!-- @elseif($data->status == 'ACCEPT' && $data->nik_peminjam == Auth::User()->nik)
                        <button class="btn btn-md btn-success" data-target="#ambil_modal" onclick="ambil('{{$data->id_barang}}','{{$data->id_transaksi}}')" data-toggle="modal"  style="text-align: center;width: 75px">AMBIL</button> -->
                        @else
                ...
                <table class="table table-bordered nowrap DataTable" id="datatable" width="100%" cellspacing="0">
                  ...
                  <tbody id="products-list" name="products-list">
                    ...
                    <tr>
                      ...
                      <!-- <td>
                        @if($data->status == 'AMBIL' && $data->nik_peminjam == Auth::User()->nik)
                        <button class="btn btn-md btn-danger" data-target="#kembali_modal" data-toggle="modal" onclick="kembali('{{$data->id_barang}}','{{$data->id_transaksi}}')" style="text-align: center;width: 75px">KEMBALI</button>
                        @elseif($data->status == 'ACCEPT' && $data->nik_peminjam == Auth::User()->nik)
                        <button class="btn btn-md btn-success" data-target="#ambil_modal" onclick="ambil('{{$data->id_barang}}','{{$data->id_transaksi}}')" data-toggle="modal"  style="text-align: center;width: 75px">AMBIL</button>
                        @else
                        <button class="btn btn-md btn-success disabled" style="text-align: center;width: 75px">AMBIL</button>
                        @endif
                      </td> -->
                      <!-- <td>
                        <button class="btn btn-sm btn-primary fa fa-pencil fa-lg" data-target="#modaledit" data-toggle="modal" style="width: 40px;height: 40px;text-align: center;">
...
<div class="modal fade" id="peminjaman" role="dialog">
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="POST" action="{{url('update_peminjaman_asset')}}" id="modalProgress" name="modalProgress">
            @csrf
          ...
          <div class="form-group">
            ...
            <select class="form-control" style="width: 100%" id="peminjam" name="peminjam" required>
...
<div class="modal fade" id="accept_modal" role="dialog">
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        <div class="modal-body">
          <form method="POST" action="{{url('accept_pinjam_warehouse')}}" id="modalProgress" name="modalProgress">
            @csrf
...
<div class="modal fade" id="reject_modal" role="dialog">
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        <div class="modal-body">
          <form method="POST" action="{{url('reject_pinjam_warehouse')}}" id="modalProgress" name="modalProgress">
            @csrf
...
<div class="modal fade" id="ambil_modal" role="dialog">
  ...
  <div class="modal fade" id="kembali_modal" role="dialog">
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        <div class="modal-body">
          <form method="POST" action="{{url('kembali_pinjam_warehouse')}}" id="modalProgress" name="modalProgress">
            @csrf
