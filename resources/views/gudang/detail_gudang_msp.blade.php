...
<section class="content-header">
  <h1>
    Detail Inventory MSP - <span class="btn-success">{{$datak->kode_barang}}</span>
  </h1>
  <ol class="breadcrumb">
    <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
    <li class="active"><a href="{{url('inventory/msp')}}">Inventory</a></li>
  ...
  <div class="box">
    ...
    <div class="box-body">
      <table class="table table-bordered">
        ...
        <tr>
          <td>Current Stock</td>
          <td>: {{$datak->qty}} {{$datak->unit}}</td>
        </tr>
        <tr>
          <td>Last Activity</td>
          <td>:
            @if(isset($keg))
              @if($keg->status == 'P' && $notes == 'FA') 
                Stock Received 
              @elseif($keg->status == 'W') 
                Stock Returned 
              @elseif($keg->status == 'D' && $notes == 'SJ') 
                Delivery Order 
              @else {{$notes}} @endif at {{date('F, d - Y', strtotime($dating))}}
            @else
              Return Product at {{date('F, d - Y', strtotime($datak->created_at))}}
            @endif
          </td>
        </tr>
      </table>
