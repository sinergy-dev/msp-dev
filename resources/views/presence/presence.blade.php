	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$.ajax({
				...
				url:"{{env('API_SIMS')}}/api/presence/getPresenceParameter",
				// url:"{{env('/presence/getPresenceParameter')}}",
				success: function(result){	
					console.log(result)
					presenceLocation = result
					// console.log(presenceLocation)
			...
			if(isLocationSet(presenceLocation)){
				isOnLocation(presenceLocation).then((result) => {
					...
					if(result){
						$.ajax({
							type:"POST",
							url:"{{env('API_SIMS')}}/api/presence/checkIn",
		...
		function checkOut(){
			Swal.fire({
				...
				if(result.value){
					...
					$.ajax({
						type:"POST",
						url:"{{env('API_SIMS')}}/api/presence/checkOut",
						data:{
							presence_actual:moment().format("YYYY-MM-DD HH:mm:ss"),
							nik:'{{Auth::User()->nik}}'
						},
